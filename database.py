    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("""
            CREATE TABLE IF NOT EXISTS raid_notify (
                ...
                times TEXT DEFAULT '19:40,19:45,19:50,19:55',
        ...
        await db.commit()
        try:
            await db.execute("ALTER TABLE reaction_roles ADD COLUMN guild_id INTEGER")
        except:
            pass
        try:
...
async def add_reaction_role(guild_id: int, message_id: int, emoji: str, role_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT OR REPLACE INTO reaction_roles (guild_id, message_id, emoji, role_id) VALUES (?, ?, ?, ?)", (guild_id, message_id, emoji, role_id))
        await db.commit()
...
async def get_all_reaction_roles(guild_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
...
async def add_wiki_entry(guild_id: int, item_id: str, info: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT OR REPLACE INTO wiki_entries (guild_id, item_id, info) VALUES (?, ?, ?)", (guild_id, item_id, info))
        await db.commit()
...
async def get_all_wiki_entries(guild_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT item_id, info FROM wiki_entries WHERE guild_id=?", (guild_id,)) as cursor:
            rows = await cursor.fetchall()
            return [{"item_id": r[0], "info": r[1]} for r in rows]
...
async def add_excused_absence(guild_id: int, user_id: int, start_date: str, end_date: str, reason: str, created_by: int, ticket_channel_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            INSERT INTO excused_absences (guild_id, user_id, start_date, end_date, reason, created_by, ticket_channel_id, created_at)
            VALUES (?,?,?,?,?,?,?,?)
...
async def get_excused_absences(guild_id: int, user_id: int = None, date: str = None):
    async with aiosqlite.connect(DB_PATH) as db:
        if user_id and date:
            async with db.execute("""
                SELECT id, user_id, start_date, end_date, reason, created_by, ticket_channel_id
                FROM excused_absences
                WHERE guild_id=? AND user_id=? AND date(?) BETWEEN start_date AND end_date
            """, (guild_id, user_id, date)) as cursor:
                rows = await cursor.fetchall()
        elif user_id:
            async with db.execute("""
                SELECT id, user_id, start_date, end_date, reason, created_by, ticket_channel_id
                FROM excused_absences
                WHERE guild_id=? AND user_id=? ORDER BY start_date DESC
            ...
            async with db.execute("""
                SELECT id, user_id, start_date, end_date, reason, created_by, ticket_channel_id
                FROM excused_absences
                WHERE guild_id=? AND date(?) BETWEEN start_date AND end_date
            """, (guild_id, date)) as cursor:
                rows = await cursor.fetchall()
        else:
            async with db.execute("""
                SELECT id, user_id, start_date, end_date, reason, created_by, ticket_channel_id
                FROM excused_absences
                WHERE guild_id=? ORDER BY start_date DESC
            """, (guild_id,)) as cursor:
                rows = await cursor.fetchall()
