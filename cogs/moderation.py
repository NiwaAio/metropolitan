...
class Moderation(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if filter_enabled:
            if contains_bad_words(message.content):
                ...
                await message.channel.send(f"{message.author.mention}, не используйте нецензурную лексику.", delete_after=5)
                return
            if contains_invite(message.content):
                await message.delete()
                await message.channel.send(f"{message.author.mention}, приглашения на другие сервера запрещены.", delete_after=5)
                return
    ...
    async def slash_warn(self, interaction: discord.Interaction, member: discord.Member, reason: str = "Не указана"):
        ...
        if new_count >= config.MAX_WARNINGS:
            ...
            if excommunicated_role:
                await member.add_roles(excommunicated_role, reason=f"Достигнуто {config.MAX_WARNINGS} предупреждений")
                await interaction.followup.send(
                    f"{member.mention} отлучён от братства. Чтобы оспорить решение, перейдите в канал {interaction.guild.get_channel(config.APPEAL_CHANNEL_ID).mention}",
                    ephemeral=True
                )
                await self.log_to_channel(interaction.guild, f"⛔ {member} отлучён за {config.MAX_WARNINGS} предупреждений.")
            else:
                await member.ban(reason=f"Достигнуто {config.MAX_WARNINGS} предупреждений")
                await interaction.followup.send(f"{member.mention} забанен за {config.MAX_WARNINGS} предупреждения (роль отлучения не найдена).", ephemeral=True)
                await self.log_to_channel(interaction.guild, f"🚫 {member} забанен за {config.MAX_WARNINGS} предупреждений (роль отлучения отсутствует).")
